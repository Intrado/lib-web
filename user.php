...
if (! $makeNewUser)
	if (QuickQuery ( "select count(*) from user where login = 'schoolmessenger' and id =?", false, array (
			$id 
	) ))
		redirect ( 'unauthorized.php' );
...
$profilename = QuickQuery ( "select name from access where id=?", false, array (
		$edituser->accessid 
) );
...
$userjobtypeids = QuickQueryList ( "select id from jobtype where id in (select jobtypeid from userjobtypes where userid=?) and not deleted and not issurvey order by systempriority, name asc", false, false, array (
		$edituser->id 
) );
...
$usersurveytypes = QuickQueryList ( "select id from jobtype where id in (select jobtypeid from userjobtypes where userid=?) and not deleted and issurvey order by systempriority, name asc", false, false, array (
		$edituser->id 
) );
...
$userfeedcategories = QuickQueryList ( "select id from feedcategory where id in (select feedcategoryid from userfeedcategory where userid=?) and not deleted", false, false, array (
		$edituser->id 
) );
...
$formdata ["login"] = array (
		...
		"validators" => array (
				...
				array (
						"ValLength",
						"min" => getSystemSetting ( "usernamelength", 5 ),
						"max" => 255 
				),
				array (
						"ValLogin",
...
if (! getSystemSetting ( '_hascallback', false )) {
	...
	$formdata ["callerid"] = array (
			...
			"control" => array (
					...
					"maxlength" => 20,
					"size" => 15,
					"selectvalues" => $authorizedcallerids,
					"allowedit" => true 
			),
			"helpstep" => 1 
	);
	...
	if (getSystemSetting ( "requireapprovedcallerid", false )) {
		$authorizedusercallerids = QuickQueryList ( "select callerid from authorizedusercallerid where userid=?", false, false, array (
				$edituser->id 
		) );
		...
		$formdata ["restrictcallerid"] = array (
				...
				"control" => array (
						"RestrictedValues",
						"values" => $authorizedcallerids,
						"label" => _L ( "Allow the following Caller IDs:" ) 
				),
				"helpstep" => 1 
		);
	}
...
$formdata ["accessid"] = array (
		...
		"validators" => array (
				...
				array (
						"ValInArray",
						"values" => array_keys ( $accessprofiles ) 
				) 
		),
		"control" => array (
				"SelectMenu",
				"values" => array (
						0 => "--- " . escapehtml ( _L ( "Select a profile" ) ) . " ---" 
				) + $accessprofiles 
		),
		"helpstep" => 2 
);
...
if (getSystemSetting ( "_hasfeed", false )) {
	$formdata ["feedcategories"] = array (
			...
			"validators" => array (
					array (
							"ValInArray",
							"values" => array_keys ( $feedcategories ) 
					) 
			),
			"control" => array (
...
if ($hasenrollment) {
	...
	if ($hasstaffid) {
		$formdata ["staffpkey"] = array (
				...
				"control" => array (
						"FormHtml",
						"html" => '<div style="border: 1px dotted gray; padding 3px">' . $edituser->staffpkey . "</div>" 
				),
				"helpstep" => 2 
		);
	} else {
	...
	$formdata ["submit"] = array (
			...
			"control" => array (
					"InpageSubmitButton",
					"name" => (($hasstaffid) ? _L ( 'Remove Staff ID' ) : _L ( 'Set Staff ID' )),
					"icon" => (($hasstaffid) ? "cross" : "disk") 
			),
			"helpstep" => 1 
	);
	...
	$userimportsections = QuickQueryList ( "
		...
		from userassociation ua
			inner join section s on
				(ua.sectionid = s.id)
		where ua.userid = ? and ua.importid is not NULL order by skey", false, false, array (
			$edituser->id 
	) );
	...
	$usersections = QuickQueryList ( "select sectionid from userassociation where userid = ? and type = 'section' and importid is null and sectionid != 0", false, false, array (
			$edituser->id 
	) );
...
$userimportorgs = QuickQueryList ( "
	...
	where ua.userid = ? and i.updatemethod = 'full' order by orgkey", false, false, array (
		$edituser->id 
) );
if ($userimportorgs) {
	$html = '<div style="border: 1px dotted gray; padding: 3px;">';
...
$userorgs = QuickQueryList ( "select organizationid from userassociation ua
		...
		and (ua.importid is null or i.updatemethod is null or i.updatemethod != 'full')", false, false, array (
		$edituser->id 
) );
...
$linkusers = QuickQueryList ( "select id, concat(firstname,' ', lastname) from user where id!=? and enabled=1 and login!='schoolmessenger' and accessid is not null order by firstname,lastname", true, false, array (
		$edituser->id 
) );
$userlinks = QuickQueryList ( "select subordinateuserid from userlink where userid=?", false, false, array (
		$edituser->id 
) );
...
if ($hasstaffid) {
	$formdata ["datarules"] ["validators"] [0] ["allowedFieldTypes"] = array (
			'f',
			'g' 
	);
	$formdata ["datarules"] ["control"] ["allowedFieldTypes"] = array (
			'f',
			'g' 
	);
}
...
if ($readonly) {
	// First Name
	$formdata ["firstname"] ["control"] = array (
			"FormHtml",
			"html" => $edituser->firstname 
	);
	unset ( $formdata ["firstname"] ["validators"] );
	// Last Name
	$formdata ["lastname"] ["control"] = array (
			"FormHtml",
			"html" => $edituser->lastname 
	);
	unset ( $formdata ["lastname"] ["validators"] );
	// Description
	unset ( $formdata ["description"] );
	// User Login
	$formdata ["login"] ["control"] = array (
			"FormHtml",
			"html" => $edituser->login 
	);
	...
	// Use LDAP login auth
	if ($IS_LDAP)
		$formdata ["ldap"] ["control"] = array (
				"FormHtml",
				"html" => $ldapuser ? "True" : "False" 
		);
		// Use SAML login auth
	if ($IS_SAML)
		$formdata ["samlEnabled"] ["control"] = array (
				"FormHtml",
				"html" => $samluser ? "True" : "False" 
		);
		// Password
	unset ( $formdata ["password"] ["requires"] );
	// Access code
	$formdata ["accesscode"] ["control"] = array (
			"FormHtml",
			"html" => $edituser->accesscode 
	);
	unset ( $formdata ["accesscode"] ["validators"] );
	// Phone login PIN
	unset ( $formdata ["pin"] ["requires"] );
	// Account Email
	$formdata ["email"] ["control"] = array (
			"FormHtml",
			"html" => $edituser->email 
	);
	unset ( $formdata ["email"] ["validators"] );
	// Autoreport Email
	$formdata ["aremail"] ["control"] = array (
			"FormHtml",
			"html" => $edituser->aremail 
	);
	unset ( $formdata ["aremail"] ["validators"] );
	// Phone
	$formdata ["phone"] ["control"] = array (
			"FormHtml",
			"html" => Phone::format ( $edituser->phone ) 
	);
	...
	// Caller ID
	if (! getSystemSetting ( '_hascallback', false )) {
		$formdata ["callerid"] ["control"] = array (
				"FormHtml",
				"html" => ($edituser->id + 0 > 0) ? Phone::format ( $edituser->getSetting ( "callerid", "" ) ) : "" 
		);
		unset ( $formdata ["callerid"] ["validators"] );
	}
	// Access Profile
	$formdata ["accessid"] ["control"] = array (
			"FormHtml",
			"html" => $profilename 
	);
	...
	$formdata ["jobtypes"] ["control"] = array (
			"FormHtml",
			"html" => "<div style='border: 1px dotted;'>$displayjobtypes</div>" 
	);
	...
	if (getSystemSetting ( "_hassurvey", true )) {
		...
		$formdata ["surveytypes"] ["control"] = array (
				"FormHtml",
				"html" => "<div style='border: 1px dotted;'>$displaysurveytypes</div>" 
		);
	...
	// Data restrictions
	if (count ( $rules )) {
		$formdata ["datarules"] ["control"] = array (
				"FormRuleWidget",
				"readonly" => true 
		);
...
$buttons = array (
		submit_button ( _L ( "Done" ), "submit", "tick" ),
...
if ($button = $form->getSubmit ()) { // checks for submit and merges in post data
	...
	if ($form->checkForDataChange ()) {
		...
		if (! $readonly) {
			...
			if (isset ( $postdata ['restrictcallerid'] )) {
				if (count ( $postdata ['restrictcallerid'] )) {
					if (count ( array_diff ( $postdata ['restrictcallerid'], $authorizedusercallerids ) ) > 0 || count ( array_diff ( $authorizedusercallerids, $postdata ['restrictcallerid'] ) ) > 0) {
						QuickUpdate ( "delete from authorizedusercallerid where userid=?", false, array (
								$edituser->id 
						) );
				...
				foreach ( $rules as $rule ) {
					...
					if ($hasstaffid) {
						if (substr ( $rule->fieldnum, 0, 1 ) !== "c") {
							Query ( "delete from rule where id=?", false, array (
									$rule->id 
							) );
							Query ( "delete from userassociation where userid=? and ruleid=?", false, array (
									$edituser->id,
									$rule->id 
							) );
						}
			...
			if ($button == 'inpagesubmit' && $hasstaffid) {
				...
				$edituser->staffpkey = "";
				Query ( "delete from userassociation where userid=? and sectionid=0", false, array (
						$edituser->id 
				) );
			}
			...
			if (! $hasstaffid && isset ( $postdata ['staffpkey'] ) && strlen ( $postdata ['staffpkey'] )) {
				...
				Query ( "insert into userassociation (userid, type, sectionid) values (?, 'section', 0)", false, array (
						$edituser->id 
				) );
			...
			if (count ( $datarules )) {
				foreach ( $datarules as $datarule ) {
					...
					Query ( "insert into userassociation (userid, type, ruleid) values (?, 'rule', ?)", false, array (
							$edituser->id,
							$rule->id 
					) );
				}
			...
			QuickUpdate ( "delete from userjobtypes where userid =?", false, array (
					$edituser->id 
			) );
			
			if (count ( $postdata ['jobtypes'] ))
				foreach ( $postdata ['jobtypes'] as $type )
					QuickUpdate ( "insert into userjobtypes values (?, ?)", false, array (
							$edituser->id,
							$type 
					) );
		...
		if (getSystemSetting ( "_hasfeed", false )) {
			QuickUpdate ( "delete from userfeedcategory where userid =?", false, array (
					$edituser->id 
			) );
		...
		if ($hasenrollment) {
			...
			QuickUpdate ( "delete from userassociation where userid = ? and importid is null and type = 'section' and sectionid != 0", false, array (
					$edituser->id 
			) );
			// re add new user associations
			$sectionids = (isset ( $postdata ['sectionids'] ) ? $postdata ['sectionids'] : array ());
			foreach ( $sectionids as $sectionid )
				QuickUpdate ( "insert into userassociation (userid, type, sectionid) values (?, 'section', ?)", false, array (
						$edituser->id,
						$sectionid 
				) );
		}
		...
		QuickUpdate ( "delete ua from userassociation ua left join import i on (i.id = ua.importid) where ua.userid = ? and ua.type = 'organization' and ua.organizationid != 0 and (ua.importid is null or i.updatemethod is null or i.updatemethod != 'full')", false, array (
				$edituser->id 
		) );
		if (isset ( $postdata ['organizationids'] ))
			foreach ( $postdata ['organizationids'] as $orgid )
				QuickUpdate ( "insert into userassociation (userid, type, organizationid) values (?, 'organization', ?)", false, array (
						$edituser->id,
						$orgid 
				) );
		...
		if ($button == 'inpagesubmit') {
			...
			notice ( _L ( "Changes to account %s are now saved", SmartTruncate($edituser->login,50) ) );
