...
var DatePickerUtils = {
	...
	dateObjectToAnsi: function(dateObj) {
		...
		return dateObj.getFullYear().toPaddedString(4) + '-' + (dateObj.getMonth() + 1).toPaddedString(2) + '-' + dateObj.getDate().toPaddedString(2);
	},
	...
	noDatesBefore: function (firstDate) {
		return new DatePickerFilter(
			function(year, month) {
				...
				var testDate = DatePickerUtils.dateObjectToAnsi(DatePickerUtils.parseDate(firstDate));
			...
			function(year, month) {
				/* Perform our date comparisons with ANSI/ISO date strings */
				var testDate = DatePickerUtils.dateObjectToAnsi(DatePickerUtils.parseDate(firstDate));
				var calDate = DatePickerUtils.yearMonthToAnsiStub(year,month) + DatePickerUtils.getMonthDays(year,month);
...
DatePicker.prototype    = {
	...
	initialize    : function ( h_p ) {
		...
		Event.observe(this._relative, 'focus', this.click.bindAsEventListener(this), false);
	...
	load: function () {
		...
		Event.observe($(this._id_datepicker_prev),
			'mousedown',
			this.prevMonth.bindAsEventListener(this),
			false);
		Event.observe($(this._id_datepicker_next),
			'mousedown', 
			this.nextMonth.bindAsEventListener(this), 
			false);
			...
			for ( var j = 0; j < 7; j++ ) {
				a_d[i][j] = {
					...
					c : ( switchNextMonth )? 'outbound' :
						(((daysIndex == this._todayDate.getDate()) && (currentMonth  == this._todayDate.getMonth()) && (currentYear == this._todayDate.getFullYear())) ? 'today' : null),
	...
	_maybeRedrawMonth : function(a_new) {
		...
		if (!this._dateFilter.validMonthP || this._dateFilter.validMonthP(_newYear, _newMon)) {
