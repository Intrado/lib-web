...
( function() {
	...
	CKEDITOR.plugins.add( 'attachmentlink', {
		...
		icons: 'attachmentlink', // %REMOVE_LINE_CORE%
		...
		//	if ( editor.plugins.image2 )
			...
			var allowed = 'div,img[alt,!src]{border-style,border-width,float,height,margin,margin-bottom,margin-left,margin-right,margin-top,width}',
				required = 'div,img[alt,src]';

			if ( CKEDITOR.dialog.isTabEnabled( editor, pluginName, 'advanced' ) )
				allowed = 'div,img[alt,dir,id,lang,longdesc,!src,title]{*}(*)';
			...
			editor.on( 'doubleclick', function( evt ) {
				...
				if ( element.is( 'img' ) && !element.data( 'cke-realelement' ) && !element.isReadOnly() )
					evt.data.dialog = 'attachmentlinkd';
			} );
			...
			if ( editor.addMenuItems ) {
				editor.addMenuItems( {
					image: {
						...
						command: 'attachmentlink',
						group: 'attachmentlink'
					}
				} );
			}
