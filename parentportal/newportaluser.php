...
?>
	<form method="POST" action="newportaluser.php<?echo $appendcustomerurl;?>" name="newaccount" onsubmit='if(!document.newaccount.acceptterms.checked){ window.alert("You must accept the Terms of Service."); return false;}'>
			...
			<fieldset>
				<input type="checkbox" id="form_textme" name="notifysms" value="1" <?=$notifysms ? "checked" : "" ?> />
				<label for="form_textme" class="check">&nbsp;<?=_L("Text me when I have a new phone message.")?></label>
			</fieldset>
			
			<fieldset>
				<label id="label_phone" class="disabled" for="form_phone"><?=_L("Mobile Phone for SMS Text")?>:</label>
				<input type="text" id="form_phone" class="disabled" name="sms" value="<?=Phone::format($sms)?>" size="20" maxlength="20" <?=$notifysms ? "" : "disabled=\"true\"" ?>/>
			</fieldset>
