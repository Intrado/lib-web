...
function fmt_actions($row, $index) {
	...
	if ($MANAGERUSER->authorized("editcustomer"))
		$actions .= '<a href="customereditcommsuite.php?id=' . $row[0] .'" title="Edit"><img src="mimg/s-edit.png" border=0></a>';
...
$query = "select c.id, c.shardid, c.urlcomponent, c.oem, c.oemid, c.nsid, c.notes, c.enabled, c.inboundnumber from customer c inner join customerproduct p on (p.customerid = c.id and product = 'cs') where $sqlsearch $sqltoggledisabled $favidsql order by id";
