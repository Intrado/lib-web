...
$buttons = array(submit_button(_L("Save"),"save","tick"),submit_button(_L("Save and Return"),"done","tick"),
				icon_button(_L('Cancel'),"cross",null,"commsuitecustomers.php"));
...
if ($button = $form->getSubmit()) { //checks for submit and merges in post data
	...
	if ($form->checkForDataChange()) {
		...
		if($button == "done") {
			if ($ajax)
				$form->sendTo("commsuitecustomers.php");
			else
				redirect("commsuitecustomers.php");
		} else {
			if ($ajax)
				$form->sendTo("customereditcommsuite.php");
			else
				redirect("customereditcommsuite.php");
		}
	}
}
