...
if (isset($_SESSION['personid']) &&
	DBFind("Person", "from person where id=" . $_SESSION['personid'])) {
	// editing existing person
	$data = DBFind("PersonData", "from persondata where personid = " . $_SESSION['personid']);
	$address = DBFind("Address", "from address where personid = " . $_SESSION['personid']);

	$phones = DBFindMany("Phone", "from phone where personid=" . $_SESSION['personid'] . " order by sequence");
	$emails = DBFindMany("Email", "from email where personid=" . $_SESSION['personid'] . " order by sequence");
} else {
	// creating new person
	clearDataFields();
}
