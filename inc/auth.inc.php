...
function getCustomerName($url) {
	...
	$result = callxmlrpc($method, $params);
	if ($result !== false) {
		// success
...
function getSessionData($id) {
	$params = array(XMLRPC_prepare($id));
...
function putSessionData($id, $sess_data) {
	$params = array(XMLRPC_prepare($id), $sess_data);
...
function callxmlrpc($method, $params) {
	global $SETTINGS;
	$result = XMLRPC_request($SETTINGS['authserver']['host'], $SETTINGS['authserver']['path'], $method, $params);
	if ($result[0] === false) {
		error_log($result[1]);
...
// DO NOT ALTER CODE WITHIN KEITH DEVENS LIBRARY
//////////////////////////////////////////////////////////////////////////////////////////////////////////
?>
