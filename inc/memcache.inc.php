...
class MemcachePoolExt extends MemcachePool {
	 ...
	 * 					If this is the case, they must be convertable to strings. 
	 * 					If generated key becomes too large, is it truncated and portions are replaced with a hash.
	 * 					If key was specified, it must be <= 250 chars 
	...
	function gen2cache ($exptime, $key = null, $callback /*, arg1, arg2 */) {
		...
		if ($key === null) {
			$key = $this->callback2key($callback, $args);
		}
		...
		while (microtime(true) - $starttime < $max_lock_time) {
			...
			} else if ($this->add($key.".lock", $lock_id, 0, $max_lock_time)) {
				//if lock accuired, generate and put
				$data = call_user_func_array($callback, $args);
				$this->set($key, $data, 0, $exptime);
				//delete lock
	...
	function gen2cache_invalidate($callback /*, arg1, arg2 */) {
		...
		$this->delete(callback2key($callback, $args));
	}
	 ...
	 * Args must be convertable to strings.
	 * Doesn't work with create_function() or closures as they can vary at run-time.
	 */
	function callback2key ($callback, $args) {
		//handle object method callbacks, ie array('MyClass', 'myCallbackMethod')
		if (is_array($callback))
			$callback = implode("::", $callback);
			
		$args = array_map($args,"rawurlencode"); //keys shouldn't contain spaces or control characters.
		//TODO could use http_build_query to handle arrays, nested arrays, etc.
		
		$key = "MemcachePoolExt_" . $callback . ":" . implode("|", $args);
		if (strlen($key) > 200) {
	...
	// methods from original (per README)
