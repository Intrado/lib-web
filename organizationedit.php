	...
	if ($form->checkForDataChange()) {
		...
		if (isset($originalorg) && (strtolower($originalorg->orgkey) == strtolower($orgkey))) {
			...
			if (isset($originalorg)) {
				...
				QuickUpdate("delete from setting where organizationid = ?", false, array($org->id));
				QuickUpdate("insert into setting (name, value, organizationid) select name, value, " . $org->id . " from setting where organizationid = ?", false, array($originalorg->id));
