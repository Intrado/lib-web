...
class commsuite_EmailTemplateGroup {
      ...
      switch ($fid)
      {
        case 1:
          if ($ftype == TType::MAP) {
            ...
            {
              $key5 = '';
              $val6 = new commsuite_EmailTemplate();
              $xfer += $input->readString($key5);
              $val6 = new commsuite_EmailTemplate();
              $xfer += $val6->read($input);
              $this->templates[$key5] = $val6;
            }
  ...
  public function write($output) {
    $xfer = 0;
    $xfer += $output->writeStructBegin('EmailTemplateGroup');
    if ($this->templates !== null) {
      if (!is_array($this->templates)) {
        throw new TProtocolException('Bad type in structure.', TProtocolException::INVALID_DATA);
      }
      $xfer += $output->writeFieldBegin('templates', TType::MAP, 1);
      {
        $output->writeMapBegin(TType::STRING, TType::STRUCT, count($this->templates));
        {
          foreach ($this->templates as $kiter7 => $viter8)
          {
