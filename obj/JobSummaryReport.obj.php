...
class JobSummaryReport extends ReportGenerator{
	...
	static function getPhoneInfo($joblist, $readonlyconn) {
		$phonenumberquery = "select sum(rc.type='phone') as total,
									...
									(select 100 * sum(rp2.status != 'duplicate' and rp2.iscontacted) / (sum(rp2.status != 'duplicate') + 0.00) as success_rate from reportperson rp2 where rp2.type='phone' and rp2.jobid in ('$joblist')) as success_rate
									from reportperson rp
									left join reportcontact rc on (rp.jobid = rc.jobid and rp.type = rc.type and rp.personid = rc.personid)
	...
	static function getEmailInfo($joblist, $readonlyconn) {
		$emailquery = "select sum(rc.type = 'email') as total,
									...
									sum(rp.status = 'nocontacts' and rc.result is null) as nocontacts,
									sum(rp.status = 'declined' and rc.result is null) as declined,
									(select 100 * sum(rp2.status != 'duplicate' and rp2.iscontacted) / (sum(rp2.status != 'duplicate') + 0.00) as success_rate from reportperson rp2 where rp2.type='email' and rp2.jobid in ('$joblist')) as success_rate
									from reportperson rp
									left join reportcontact rc on (rp.jobid = rc.jobid and rp.type = rc.type and rp.personid = rc.personid)
									where rp.jobid in ('$joblist')
									and rp.type='email'";
	...
	static function getSmsInfo($joblist, $readonlyconn) {
		$smsquery = "select sum(rc.type = 'sms') as total,
									...
									sum(rp.status = 'nocontacts' and rc.result is null) as nocontacts,
									sum(rp.status = 'declined' and rc.result is null) as declined,
									(select 100 * sum(rp2.status != 'duplicate' and rp2.iscontacted) / (sum(rp2.status != 'duplicate') + 0.00) as success_rate from reportperson rp2 where rp2.type='sms' and rp2.jobid in ('$joblist')) as success_rate
									from reportperson rp
									left join reportcontact rc on (rp.jobid = rc.jobid and rp.type = rc.type and rp.personid = rc.personid)
									where rp.jobid in ('$joblist')
									and rp.type='sms'";
