...
// TODO: Replace with Message once migration/db schema is finalized. For now, named Message2.obj.php for use with MessageGroupForm editor.


class Message2 extends DBMappedObject {
	var $userid;
	var $messagegroupid;
	var $name;
	var $description;
	...
	//const AUTOTRANSLATE_VALUES = array('none', 'source', 'translated', 'overridden');

	function Message2 ($id = NULL) {
		$this->_allownulls = true;
		$this->_tablename = "message";
	...
	function reset_attachments($files) {
		// Delete any existing parts, we will create a fresh set.
		QuickUpdate("delete from messageattachment where messageid=$this->id");

		foreach ($files as $contentid => $file) {
			$attachment = new MessageAttachment();
			$attachment->messageid = $this->id;
	...
	function update_with_parts($body, $voiceid) {
		// Delete any existing parts, we will create a fresh set.
		if (!is_null($this->id))
			QuickUpdate("delete from messagepart where messageid=$this->id");

		if ($this->type == 'sms') {
			$this->autotranslate = 'none';
			$this->update();
