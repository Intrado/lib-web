...
class SddReport extends ReportGenerator {
	...
	function generateQuery($hackPDF = false){
		...
		// the query return only messageattachments for which there is >= 1 row in reportsdddelivery.
...
rc.type,
ba.filename,
coalesce(sdd_download.action, sdd_action.action) as action,
...
inner join reportsdddelivery as sdd_action on (sdd_action.messageAttachmentId = ma.id)
...
inner join person as pp on (pp.id = sdd_action.personid)
inner join reportperson as rp on (rp.jobid = b.jobid and rp.type = 'email' and rp.personid = pp.id)
inner join reportcontact as rc on (rc.jobid = rp.jobid and rc.type = rp.type and rc.personid = rp.personid)
