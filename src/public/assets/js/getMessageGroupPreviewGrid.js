...
function getMessageGroupPreviewGrid(mgid, container, jobtypeid,jobid) {
	...
	container.update(new Element("img", { "src": "assets/img/ajax-loader.gif" }));
			...
			data.each(function(item) {
				...
				headers.each(function(header) {
					...
					if (item.value[header.key]) {
						hasMessage = true;
						var icon = new Element("img", { "src": "assets/img/icons/accept.png" });
					} else {
						...
						if (item.key !== defaultlang && ["smsplain","postfacebook","posttwitter","postfeed","postpage","postvoice"].indexOf(header.key) != -1)
							...
							var icon = new Element("img", { "src": "assets/img/icons/diagona/16/160.png" });
					}
