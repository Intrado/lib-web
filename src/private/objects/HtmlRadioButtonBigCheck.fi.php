	...
	function render ($value) {
		...
		foreach ($this->args['values'] as $val => $html)  {
			...
			$str .= '<div class="creation_method">
				...
				<img id="'.$id.'" name="'.$id.'" class="htmlRadioButtonBigCheck_checkImg" src="'.(($value == $val)?'assets/img/bigradiobutton_checked.gif':'assets/img/bigradiobutton.gif').'" onclick="htmlRadioButtonBigCheck_doCheck(\''.$this->form->name.'\', \''.$n.'\',  \''.$id.'\', \''.$n.'-container\', \''.$val.'\')" />
				</div>';
		}
		$str .= '
			<script>
				function htmlRadioButtonBigCheck_doCheck(form, formitem, checkimg, container, value) {
					...
					container.select(\'[class="htmlRadioButtonBigCheck_checkImg"]\').each( function(i) {
						$(i).src = "assets/img/bigradiobutton.gif";
					});
					checkimg.src = "assets/img/bigradiobutton_checked.gif";
